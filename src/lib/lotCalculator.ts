...
export function calculateLots(
  ...
  let isRounded = false;
  if (market === 'ID' && entry > 0) {
    const rounded = roundToIDXTick(entry);
    if (rounded !== entry) {
      entry = rounded;
      isRounded = true;
    }
  ...
  const pricePerLot = entry * sharesPerLot;
  ...
  const totalCost = totalShares * entry;
  ...
  // quantity = floor(riskAmount / (entry - stopLossPrice))
  ...
  const isValidStop = stop !== null && stop > 0 && stop < entry;
  const riskPerShare = isValidStop ? entry - stop! : null;
  ...
  const recommendedCost = recommendedShares * entry;
  ...
  const roundedMsg = isRounded ? ` (automatically rounded to IDX tick size)` : '';
  ...
  if (market === 'ID') {
    ...
    if (!isValidStop) {
      recommendedReason = strictRiskBasedOnly
        ...
        : `Without a stop-loss price, we can’t size the position based on “max loss per trade”. The recommendation below is limited by what you can afford. Add a stop-loss price to get a risk-based (1–2% rule) position size.${roundedMsg}`;
    } else if (actualRecommended === 0) {
      recommendedReason = `Your stop-loss is too tight relative to your risk limit: with risk ${riskPercent}% (≈ Rp${riskAmount.toLocaleString('id-ID')}) and effective stop distance Rp${effectiveRiskPerShare!.toLocaleString('id-ID')}/share${bufferPart}, the risk-based size rounds down to 0 lot. Increase risk %, widen stop-loss, or use a cheaper entry.${roundedMsg}`;
    } else {
      recommendedReason = `Recommended size is ${actualRecommended} lot(s) (${recommendedShares.toLocaleString('id-ID')} shares) for Rp${recCost.toLocaleString('id-ID')} (${recPercent.toFixed(1)}% of your capital). With entry Rp${entry.toLocaleString('id-ID')}${roundedMsg} and stop Rp${stop!.toLocaleString('id-ID')}, max loss at stop is ≈ Rp${maxLossAtStop!.toLocaleString('id-ID')} (≤ ${riskPercent}% of capital)${bufferPart}.`;
    }
  } else {
    const recCost = recommendedCost;
    const recPercent = recommendedPercent;
    ...
    if (!isValidStop) {
      ...
      recommendedReason = `Recommended size is ${actualRecommended} share(s) for $${recCost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (${recPercent.toFixed(1)}% of your capital). With entry $${entry.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} and stop $${stop!.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}, max loss at stop is ≈ $${maxLossAtStop!.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (≤ ${riskPercent}% of capital)${bufferPart}.`;
    }
  }
